/**
 * Final RLS Test Script
 * Tests RLS policies with proper authentication
 */
...
async function createUserClient(userId: string) {
  // In a real app, you would generate a proper JWT for the user
  // For testing, we'll use the service key with a custom header
...
async function testRlsPolicies() {
  ...
  try {
    ...
    console.log(chalk.yellow('\n1. Creating authenticated client...'));
    const userClient = await createUserClient(validUserId);
    console.log(chalk.green('✅ Created authenticated client'));
    ...
    const interactionData = {
      ...
      type: 'email',
      title: 'Final RLS Test',
      content: 'Testing RLS policies with proper auth',
    ...
    // First, try with the user's client
