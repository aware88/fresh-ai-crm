 ...
 * Tests basic RLS functionality without complex JWT handling
 */
...
// Create an authenticated client with a user ID
...
async function testRlsPolicies() {
  ...
  try {
    ...
    const authClient = createAuthClient(validUserId);
    ...
    const interactionData = {
      ...
      type: 'email',
      title: 'RLS Simple Test',
      content: 'Testing RLS policies with simple auth',
    ...
    if (userError) {
      ...
      if (userAccessSuccess) {
        ...
        const { data: allUserData } = await authClient
          .from('interactions')
          .select('*');
