...
-- Organization admins can view and manage user_roles for users in their organization
...
CREATE POLICY user_roles_org_admin_insert_policy ON public.user_roles
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.organization_members om1
      JOIN public.organization_members om2 ON om1.organization_id = om2.organization_id
      JOIN public.roles r ON NEW.role_id = r.id AND (r.organization_id = om1.organization_id OR r.type = 'system_admin')
      WHERE om1.user_id = auth.uid()
...
CREATE POLICY user_roles_org_admin_delete_policy ON public.user_roles
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.organization_members om1
      JOIN public.organization_members om2 ON om1.organization_id = om2.organization_id
      JOIN public.roles r ON public.user_roles.role_id = r.id AND (r.organization_id = om1.organization_id OR r.type = 'system_admin')
      WHERE om1.user_id = auth.uid()
      AND om2.user_id = public.user_roles.user_id
      AND om1.role = 'admin'
    )
