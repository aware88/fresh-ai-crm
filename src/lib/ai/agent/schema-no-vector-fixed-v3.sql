...
CREATE TABLE IF NOT EXISTS conversation_contexts (
  ...
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  agent_id UUID NOT NULL REFERENCES agent_configs(id) ON DELETE CASCADE,
  contact_id TEXT REFERENCES contacts(id) ON DELETE SET NULL, -- Changed to TEXT to match contacts.id
  conversation_id TEXT NOT NULL, -- Keep as TEXT as it's not a foreign key
  current_goal TEXT,
...
CREATE TABLE IF NOT EXISTS agent_decisions (
  ...
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  agent_id UUID NOT NULL REFERENCES agent_configs(id) ON DELETE CASCADE,
  conversation_id TEXT NOT NULL, -- Keep as TEXT to match conversation_contexts
...
CREATE TABLE IF NOT EXISTS contact_personality_profiles (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  contact_id TEXT NOT NULL REFERENCES contacts(id) ON DELETE CASCADE, -- Changed to TEXT to match contacts.id
