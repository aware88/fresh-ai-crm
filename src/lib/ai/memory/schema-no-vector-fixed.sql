-- AI Memory System Schema (Phase 1) - No Vector Extension Version
-- First, enable required extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- AI Memories Table
-- Stores AI memories with JSON embeddings for alternative semantic search
CREATE TABLE IF NOT EXISTS ai_memories (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  content TEXT NOT NULL,
  embedding_json JSONB, -- Store embedding as JSON array instead of vector
...
-- Function to calculate cosine similarity between two JSON arrays
-- This is a fallback for vector similarity when pgvector is not available
CREATE OR REPLACE FUNCTION cosine_similarity_json(embedding1 JSONB, embedding2 JSONB)
RETURNS FLOAT AS $$
DECLARE
  dot_product FLOAT := 0;
  norm1 FLOAT := 0;
...
BEGIN
  ...
  len := LEAST(jsonb_array_length(embedding1), jsonb_array_length(embedding2));
